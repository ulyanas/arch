...
public class FilterFramework extends Thread
{
	...
	****************************************************************************/
	...
	public FilterFramework(int outPutPortNum){
		...
		for(int i=0; i<=outPutPortNum-1; i++) outputWritePortList.add(new PipedOutputStream());
	}
	...
	****************************************************************************/
	...
	void Connect( FilterFramework Filter )
	{
		try
		{
			...
			InputReadPort.connect( Filter.outputWritePortList.get(0) );
			InputFilter = Filter;
	...
	* Purpose: This method connects filters to each other. All connections are
	* through the inputport of each filter. That is each filter's inputport is
	* connected to another filter's specific output port through this method.
	*
	* Arguments:
	* 	FilterFramework - this is the filter that this filter will connect to.
	* 	outPutPortIndex - this is the outPutPort in the filter this filter will connect to.
	*
	* Returns: void
	*
	* Exceptions: IOException
	*
	****************************************************************************/
	void Connect( FilterFramework Filter, int outPutPortIndex)
	{
		try
		{
			// Check whether the output port exist
			if(outPutPortIndex >= Filter.outputWritePortList.size()) throw new Exception("No such ouput port");
			// Connect this filter's input to the upstream pipe's output stream
			InputReadPort.connect(Filter.outputWritePortList.get(outPutPortIndex));
			InputFilter = Filter;
	...
	****************************************************************************/
	...
	/***************************************************************************
	* CONCRETE METHOD:: WriteFilterOutputPort
	* Purpose: This method writes data to the specific output port.
	*
	* Arguments:
	* 	byte datum - This is the byte that will be written on the output port.of
	*	the filter.
	*	int outPortIndex - the index of output port
	*
	* Returns: void
	*
	* Exceptions: IOException
	*
	****************************************************************************/
	void WriteFilterOutputPort(byte datum, int outPortIndex)
	{
		try
		{		   	
		...
		{
			System.out.println("\n" + this.getName() + " Pipe "+ outPortIndex+ " write error::" + Error );
	...
	****************************************************************************/
	...
	void ClosePorts()
	{
		try
		{
			...
			for(PipedOutputStream outWritePort : outputWritePortList) outWritePort.close();
		}
		catch( Exception Error )
		{
