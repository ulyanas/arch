...
public class PressureFilter extends FilterFramework{
	...
	public void run()
    {
		...
		while (true)
		{
			try
			{
				...
				// Wild Point
				if( record.getPressure() < 0.0 || (lastValidRecord != null && Math.abs(record.getPressure() - lastValidRecord.getPressure()) > 10)){
	...
	private void reviseWildPointList(ArrayList<Record> wildRecordList, Record lastValidRecord, Record nextValidRecord){
		double lastValidPressure = lastValidRecord == null ? 0 : nextValidRecord.getPressure();
		double nextValidPressure = nextValidRecord == null ? 0 : lastValidRecord.getPressure();
