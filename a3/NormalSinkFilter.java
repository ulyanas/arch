...
public class NormalSinkFilter extends FilterFramework {
	...
	public NormalSinkFilter(String fileName){
		super();
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				// time
				out.writeBytes(TimeStampFormat.format(record.getTimeStamp()
						.getTime()));
				out.writeBytes("\t");
				// temperature
				...
				out.writeBytes("\t");
				// pressure
				out.writeBytes(String.format("%08.5f", record.getPressure())
						.replace('.', ':'));
				if (record.getExtra() > 0)
					out.writeBytes("*");
				out.writeBytes("\t");
				out.writeBytes("\n");
			} // while
